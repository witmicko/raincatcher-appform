...
function wrapper(mediator) {
  ...
  mediator.subscribe('appform:submission:list:remote:load', function(submissionIds, id) {
    client.getSubmissions(submissionIds)
    .then(function(submissions) {
      mediator.publish('done:appform:submission:list:remote:load:'+id, submissions);
    }, function(error) {
      mediator.publish('error:appform:submission:list:remote:load:'+id, error);
    });
  });
  ...
  mediator.subscribe('appform:submission:upload', function(submission) {
    ...
    .then(function(submissionId) {
      mediator.publish('done:appform:submission:upload:' + submission.props._ludid, submissionId);
    }, function(error) {
  ...
  client.addSubmissionCompleteListener(function(submissionResult, metaData) {
    if (metaData) {
      ...
      mediator.publish('appform:submission:complete', event)
    }
  })
};
