...
ngModule.directive('appformSubmission', function($templateCache, $q, mediator) {
  ...
  , controller: function($scope) {
      ...
      submissionPromise.then(function(submission) {
        ...
        mediator.request('appform:submission:field:list', submission, {uid: submission.getLocalId()}).then(function(fields) {
          self.fields = fields;
        }, function(error) {
          console.error(error);
        });
  ...
  , controller: function($scope) {
    ...
    self.done = function(isValid) {
      ...
      if (isValid) {
        ...
        mediator.request('appform:submission:create', [submissionFields, form, ts], {uid: ts})
        .then(function(submissionResults) {
          mediator.publish('workflow:step:done', submissionResults);
        });
      };
    }
  }
