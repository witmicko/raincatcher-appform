...
client.synchSubmissionResult = function(result) {
  ...
  var self = this;
  var submissionPromise = result.submission._submission ? q.when(result.submission._submission)
    : result.submission.submissionId ? self.getSubmission(result.submission.submissionId)
    : self.getSubmissionLocal(result.submission.submissionLocalId);
  return submissionPromise.then(function(submission) {
